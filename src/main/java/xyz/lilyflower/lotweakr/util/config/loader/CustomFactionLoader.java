        ...
        for (String path : customFactionList.list()) {
            try (Stream<String> stream = Files.lines(Paths.get(customFactionList + "/" + path), StandardCharsets.UTF_8)) {
                ...
                if (array.length >= 8) {
                        ...
                        switch (lineNo++) {
                            ...
                            case 5:
                                ...
                                for (String type : factionTypes) {
                                    LOTRFaction.FactionType parsed = LOTRFaction.FactionType.valueOf(type.toUpperCase());
                                    types.add(parsed);
                                }
                    ...
                    relations.forEach((relation, factions) -> {
                        factions.forEach(fac -> {
                            LOTRFactionRelations.setDefaultRelations(faction, fac, relation);
                        });
                    });
